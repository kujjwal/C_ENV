...
int scanFile(FILE *file) {
    char *ptr;
    int num;
    char str[INREAD_INPUT_SIZE];

    fgets(str, INREAD_INPUT_SIZE, file);
...
int main(int argc, char** argv) {
    ...
    free(instructions);
    int occupied_register = find_empty_register();
    if(occupied_register == EX) {
        printf("No empty registers");
        return 0;
    } else {
        printf("Occupied register at %d", occupied_register);
        return -1;
    }
}
